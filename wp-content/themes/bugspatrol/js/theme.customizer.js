...
function bugspatrol_theme_customizer(option, val) {
"use strict";
	var custom_style = '';
	...
	if (option == 'body_style') {
		...
		var params = BUGSPATROL_STORAGE['co_add_params']!=undefined ? BUGSPATROL_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#bugspatrol-customizer-styles-'+option).length > 0 ? jQuery('#bugspatrol-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="bugspatrol-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
