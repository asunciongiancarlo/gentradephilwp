...
if ( !function_exists( 'bugspatrol_template_list_theme_setup' ) ) {
	add_action( 'bugspatrol_action_before_init_theme', 'bugspatrol_template_list_theme_setup', 1 );
	function bugspatrol_template_list_theme_setup() {
		bugspatrol_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'bugspatrol')
			));
	}
}

// Template output
if ( !function_exists( 'bugspatrol_template_list_output' ) ) {
	function bugspatrol_template_list_output($post_options, $post_data) {
